...
def _get_hierarchy_context():
    """Retorna dicionário com listas de centrais, almoxarifados, subs e setores filtrados pelo escopo do usuário."""
    ctx = {
        'centrais': [],
        'almoxarifados': [],
        'sub_almoxarifados': [],
    ...
    try:
        if extensions.mongo_db is not None:
            # Centrais
            f_centrais = ScopeFilter.get_centrais_filter()
            c_cursor = extensions.mongo_db['centrais'].find(f_centrais).sort('nome', 1)
            for d in c_cursor:
                d['id'] = str(d.get('_id'))
                ctx['centrais'].append(d)
                
            # Almoxarifados
            f_almox = ScopeFilter.get_almoxarifados_filter()
            a_cursor = extensions.mongo_db['almoxarifados'].find(f_almox).sort('nome', 1)
            for d in a_cursor:
                d['id'] = str(d.get('_id'))
                ctx['almoxarifados'].append(d)
                
            # Sub-Almoxarifados
            f_sub = ScopeFilter.get_sub_almoxarifados_filter()
            s_cursor = extensions.mongo_db['sub_almoxarifados'].find(f_sub).sort('nome', 1)
            for d in s_cursor:
                d['id'] = str(d.get('_id'))
                ctx['sub_almoxarifados'].append(d)
                
            # Setores
            f_setor = ScopeFilter.get_setores_filter()
            se_cursor = extensions.mongo_db['setores'].find(f_setor).sort('nome', 1)
            for d in se_cursor:
                d['id'] = str(d.get('_id'))
                ctx['setores'].append(d)
    except Exception as e:
        current_app.logger.error(f"Erro ao carregar contexto de hierarquia: {e}")
    return ctx
...
def configuracoes_hierarquia():
    """Página de configuração da hierarquia (Carregada do MongoDB)"""
    ctx = _get_hierarchy_context()
    return render_template('configuracoes/hierarquia.html', **ctx)
...
def produtos_detalhes(id):
    ...
    produto = None
    try:
        # Permitir edição para níveis gerenciais e superiores
        # (super_admin, admin_central, gerente_almox)
        nivel = getattr(current_user, 'nivel_acesso', None)
        can_edit = bool(getattr(current_user, 'is_authenticated', False)) and (
            nivel in ['super_admin', 'admin_central', 'gerente_almox']
        )
    except Exception:
        can_edit = False
    return render_template('produtos/detalhes.html', produto=produto, can_editar_entrada_lote=can_edit, can_edit_product=can_edit)
...
def produtos_recebimento(id):
    ...
    produto = _find_by_id('produtos', id)
    if not produto:
        flash('Produto não encontrado', 'error')
        return redirect(url_for('main.produtos_index'))
    produto['id'] = str(produto.get('_id'))
...
def estoque():
    ...
    ctx = _get_hierarchy_context()
    return render_template('produtos/estoque.html', **ctx)
...
def movimentacoes():
    ...
    ctx = _get_hierarchy_context()
    return render_template('movimentacoes/index.html', use_mongo=True, **ctx)
...
def api_estoque_hierarquia():
    """Agrega estoque por hierarquia com suporte a filtros e paginação (Otimizado)."""
    if extensions.mongo_db is None:
        return jsonify({'error': 'MongoDB não inicializado'}), 503
    ...
    # Otimização: Resolução em lote para evitar N+1 queries
    def _bulk_resolve(coll_name, ids):
        if not ids: return {}
        try:
            coll = extensions.mongo_db[coll_name]
            # Separar tipos de ID
            ...
            for x in ids:
                if not x: continue
                s_x = str(x)
                try: oids.append(ObjectId(s_x))
                except: pass
                if s_x.isdigit(): ints.append(int(s_x))
                strs.append(s_x)
            
            query = {'$or': []}
            if oids: query['$or'].append({'_id': {'$in': oids}})
            if ints: query['$or'].append({'id': {'$in': ints}})
            if strs: query['$or'].append({'id': {'$in': strs}})
            
            if not query['$or']: return {}
            
            mapping = {}
            for doc in coll.find(query):
                # Mapear por vários formatos de chave para garantir match
                if '_id' in doc:
                    mapping[str(doc['_id'])] = doc
                if 'id' in doc:
                    mapping[str(doc['id'])] = doc
            return mapping
        except Exception:
            return {}

    # Preparar candidatos de produto (lógica original mantida para filtro prévio)
    accepted_prod_ids = []
    if produto_filtro:
        if produto_filtro.isdigit():
            try: accepted_prod_ids.append(int(produto_filtro))
            except: pass
        try: accepted_prod_ids.append(ObjectId(produto_filtro))
        except: pass
        accepted_prod_ids.append(produto_filtro)
        
        try:
            produtos_coll = extensions.mongo_db['produtos']
            # Otimização: Projetar apenas ID
            ...
            for p in prods:
                if p.get('id') is not None: accepted_prod_ids.append(p['id'])
    ...
    try:
        ...
        projection = {
            ...
            'almoxarifado_id': 1, 'central_id': 1, 'local_tipo': 1,
            'local_id': 1, 'local_nome': 1, 'nome_local': 1,
            'quantidade': 1, 'quantidade_atual': 1, 'quantidade_reservada': 1,
            'quantidade_disponivel': 1, 'quantidade_inicial': 1,
            'updated_at': 1, 'data_atualizacao': 1
        }
        ...
        if no_pagination:
            ...
            batch_limit = min(per_page * 5, 500) # Aumentado para reduzir chance de página vazia
            cursor = coll.find(query, projection).sort('updated_at', -1).skip(skip).limit(batch_limit)
            
        # Buscar dados em memória para processamento em lote
        items_raw = list(cursor)
        
        # 1. Coletar IDs para Bulk Load
        prod_ids_to_fetch = set()
        loc_ids_to_fetch = {
            'centrais': set(),
            'almoxarifados': set(),
        ...
        for s in items_raw:
            if s.get('produto_id'): prod_ids_to_fetch.add(s.get('produto_id'))
            
            # Identificar local e ID
            l_id = None
            l_coll = None
            if s.get('setor_id'): 
                l_id = s.get('setor_id'); l_coll = 'setores'
            elif s.get('sub_almoxarifado_id'): 
                l_id = s.get('sub_almoxarifado_id'); l_coll = 'sub_almoxarifados'
            elif s.get('almoxarifado_id'): 
                l_id = s.get('almoxarifado_id'); l_coll = 'almoxarifados'
            elif s.get('central_id'): 
                l_id = s.get('central_id'); l_coll = 'centrais'
            else:
                lt = s.get('local_tipo')
                l_id = s.get('local_id')
                if l_id:
                    if lt == 'setor': l_coll = 'setores'
                    elif lt in ('subalmoxarifado', 'sub_almoxarifado'): l_coll = 'sub_almoxarifados'
                    elif lt == 'almoxarifado': l_coll = 'almoxarifados'
                    elif lt == 'central': l_coll = 'centrais'
            
            if l_id and l_coll:
                loc_ids_to_fetch[l_coll].add(l_id)

        # 2. Executar Bulk Queries
        prod_map = _bulk_resolve('produtos', prod_ids_to_fetch)
        loc_maps = {
            k: _bulk_resolve(k, v) for k, v in loc_ids_to_fetch.items()
        }

        # 3. Iterar e montar resultado
        for s in items_raw:
            # Produto
            raw_pid = s.get('produto_id')
            pdoc = prod_map.get(str(raw_pid))
            
            produto_nome = (pdoc or {}).get('nome') or '-'
            produto_codigo = (pdoc or {}).get('codigo') or '-'
            produto_id_out = (pdoc or {}).get('id')
            if produto_id_out is None and pdoc:
                produto_id_out = str(pdoc.get('_id'))
            if produto_id_out is None:
            ...
            if s.get('setor_id') is not None:
                tipo = 'setor'; local_id = s.get('setor_id'); coll_name = 'setores'
            elif s.get('sub_almoxarifado_id') is not None:
                tipo = 'subalmoxarifado'; local_id = s.get('sub_almoxarifado_id'); coll_name = 'sub_almoxarifados'
            elif s.get('almoxarifado_id') is not None:
                tipo = 'almoxarifado'; local_id = s.get('almoxarifado_id'); coll_name = 'almoxarifados'
            elif s.get('central_id') is not None:
                tipo = 'central'; local_id = s.get('central_id'); coll_name = 'centrais'
            else:
                tipo = s.get('local_tipo') or 'almoxarifado'
                local_id = s.get('local_id')
                if tipo == 'setor': coll_name = 'setores'
                elif tipo in ('subalmoxarifado', 'sub_almoxarifado'): coll_name = 'sub_almoxarifados'
                elif tipo == 'almoxarifado': coll_name = 'almoxarifados'
                elif tipo == 'central': coll_name = 'centrais'

            local_nome = s.get('local_nome') or s.get('nome_local') or 'Local'
            
            if coll_name and local_id is not None:
                ldoc = loc_maps.get(coll_name, {}).get(str(local_id))
                if ldoc:
                    local_nome = ldoc.get('nome') or ldoc.get('descricao') or local_nome
                    lid_out = ldoc.get('id')
                    if lid_out is None: lid_out = str(ldoc.get('_id'))
                    local_id = lid_out
            ...
            if produto_filtro:
                ...
                try: id_ok = id_ok or (str(ObjectId(produto_filtro)) == str(raw_pid))
                except: pass
                if not (nome_ok or cod_ok or id_ok): continue

            if tipo_filtro and normalize_tipo(tipo) != normalize_tipo(tipo_filtro): continue
            if local_filtro and str(local_id) != str(local_filtro): continue

            if status_filtro:
                limiar_baixo = max(inicial * 0.1, 5)
                if status_filtro == 'zerado' and not (disponivel <= 0): continue
                elif status_filtro == 'baixo' and not (disponivel > 0 and disponivel <= limiar_baixo): continue
                elif status_filtro in ('disponivel', 'normal') and not (disponivel > limiar_baixo): continue

            # Permissão de acesso
            try:
                if getattr(current_user, 'nivel_acesso', None) in ('admin_central', 'gerente_almox', 'resp_sub_almox', 'operador_setor'):
                    if not current_user.can_access_produto(produto_id_out): continue
            except: continue
        ...
        current_app.logger.error(f"Erro em api_estoque_hierarquia: {e}")
        items_all = []

    # Paginação final (corrigida)
    total = len(items_all)
    if no_pagination:
        items = items_all
        pagination = {'page': 1, 'per_page': total, 'pages': 1, 'total': total}
    else:
        # items_all já contém apenas os itens do batch atual que passaram no filtro.
        # Não devemos aplicar 'skip' novamente (start:end), pois o cursor já fez skip.
        # Apenas limitamos ao tamanho da página.
        items = items_all[:per_page]
        
        pagination = {
            'page': page,
            'per_page': per_page,
            'pages': total_pages_base, # Melhor estimativa baseada no count total do DB
            'total': base_total # Total estimado
        }

    result = {'items': items, 'pagination': pagination}
    try: extensions.response_cache.set(cache_key, result, ttl=10)
    except: pass
...
def api_estoque_hierarquia_export():
    ...
    Gera CSV compatível com Excel (Otimizado).
    """
    if extensions.mongo_db is None:
        return "Erro: MongoDB não inicializado", 503
    ...
    def _bulk_resolve(coll_name, ids):
        ...
        try:
            ...
            mapping = {}
            for doc in coll.find(query):
                if '_id' in doc: mapping[str(doc['_id'])] = doc
                if 'id' in doc: mapping[str(doc['id'])] = doc
            return mapping
        except Exception:
            return {}

    # Preparar candidatos de produto
    accepted_prod_ids = []
    if produto_filtro:
        if produto_filtro.isdigit():
            try: accepted_prod_ids.append(int(produto_filtro))
    ...
    try:
        ...
        prod_ids_to_fetch = set()
        loc_ids_to_fetch = {
            'centrais': set(), 'almoxarifados': set(),
            'sub_almoxarifados': set(), 'setores': set()
        }
        ...
        for s in items_raw:
            ...
            l_id = None
            l_coll = None
            if s.get('setor_id'): l_id = s.get('setor_id'); l_coll = 'setores'
            elif s.get('sub_almoxarifado_id'): l_id = s.get('sub_almoxarifado_id'); l_coll = 'sub_almoxarifados'
            elif s.get('almoxarifado_id'): l_id = s.get('almoxarifado_id'); l_coll = 'almoxarifados'
            elif s.get('central_id'): l_id = s.get('central_id'); l_coll = 'centrais'
            else:
                lt = s.get('local_tipo')
                l_id = s.get('local_id')
                if l_id:
            ...
            if l_id and l_coll: loc_ids_to_fetch[l_coll].add(l_id)

        # 2. Executar Bulk Queries
        prod_map = _bulk_resolve('produtos', prod_ids_to_fetch)
        loc_maps = {k: _bulk_resolve(k, v) for k, v in loc_ids_to_fetch.items()}
        ...
        for s in items_raw:
            ...
            produto_codigo = (pdoc or {}).get('codigo') or '-'
            produto_id_out = (pdoc or {}).get('id')
            if produto_id_out is None and pdoc: produto_id_out = str(pdoc.get('_id'))
            if produto_id_out is None: produto_id_out = raw_pid

            tipo = None
            local_id = None
            coll_name = None
            if s.get('setor_id') is not None: tipo = 'setor'; local_id = s.get('setor_id'); coll_name = 'setores'
            elif s.get('sub_almoxarifado_id') is not None: tipo = 'subalmoxarifado'; local_id = s.get('sub_almoxarifado_id'); coll_name = 'sub_almoxarifados'
            elif s.get('almoxarifado_id') is not None: tipo = 'almoxarifado'; local_id = s.get('almoxarifado_id'); coll_name = 'almoxarifados'
            elif s.get('central_id') is not None: tipo = 'central'; local_id = s.get('central_id'); coll_name = 'centrais'
            else:
                tipo = s.get('local_tipo') or 'almoxarifado'
                local_id = s.get('local_id')
                if tipo == 'setor': coll_name = 'setores'
        ...
        current_app.logger.error(f"Erro export: {e}")
        return "Erro ao gerar exportação", 500
...
def api_movimentacoes():
    ...
    def _get_coll_name(t):
        if not t: return None
        t = str(t).lower()
        if t in ('setor', 'setores'): return 'setores'
        if t in ('subalmoxarifado', 'sub_almoxarifado', 'sub_almoxarifados'): return 'sub_almoxarifados'
        if t in ('almoxarifado', 'almoxarifados'): return 'almoxarifados'
        if t in ('central', 'centrais'): return 'centrais'
        return None
    ...
    for m in items_raw:
        ...
        # Origem
        ot = m.get('origem_tipo') or m.get('local_tipo')
        oid = m.get('origem_id') or m.get('local_id')
        ocoll = _get_coll_name(ot)
        if ocoll and oid: loc_ids[ocoll].add(oid)
        
        # Destino
        dt = m.get('destino_tipo')
        did = m.get('destino_id')
        dcoll = _get_coll_name(dt)
        if dcoll and did: loc_ids[dcoll].add(did)

    # 2. Bulk resolve
    prod_map = _bulk_resolve('produtos', prod_ids)
    loc_maps = {k: _bulk_resolve(k, v) for k, v in loc_ids.items()}

    items = []
    for m in items_raw:
        # Escopo extra de segurança
        if restricted:
            try:
                if not current_user.can_access_produto(m.get('produto_id')): continue
            except: continue
            
        tipo_mov = (m.get('tipo') or m.get('tipo_movimentacao') or '').lower()
        
        # Produto
        raw_pid = m.get('produto_id')
        pdoc = prod_map.get(str(raw_pid))
        ...
        origem_doc = None
        if not const_prefer_supplier:
            ocoll = _get_coll_name(origem_tipo)
            if ocoll and origem_id: origem_doc = loc_maps.get(ocoll, {}).get(str(origem_id))
            
        destino_doc = None
        dcoll = _get_coll_name(destino_tipo)
        if dcoll and destino_id: destino_doc = loc_maps.get(dcoll, {}).get(str(destino_id))
        ...
        def _normalize_id(doc, raw):
            if doc:
                if doc.get('_id'): return str(doc.get('_id'))
                if doc.get('id'): return str(doc.get('id'))
        ...
        data_mov_raw = m.get('data_movimentacao') or m.get('created_at') or m.get('updated_at')
        data_mov_dt = None
        if isinstance(data_mov_raw, datetime): data_mov_dt = data_mov_raw
        elif isinstance(data_mov_raw, str):
            try: data_mov_dt = datetime.fromisoformat(data_mov_raw)
            except: data_mov_dt = None
        if data_mov_dt is None: data_mov_dt = datetime.now(timezone.utc)
        elif data_mov_dt.tzinfo is None: data_mov_dt = data_mov_dt.replace(tzinfo=timezone.utc)
        data_mov_str = data_mov_dt.isoformat()
...
@require_level('super_admin', 'admin_central', 'gerente_almox')
def api_produto_lote_entrada(produto_id, numero_lote):
    ...
    Somente 'super_admin', 'admin_central' e 'gerente_almox' podem acessar.
    """
    try:
        db = extensions.mongo_db
        coll = db['movimentacoes']
