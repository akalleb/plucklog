...
class ScopeFilter:
    ...
    def get_centrais_filter():
        """Retorna filtro MongoDB para centrais baseado no escopo do usuário"""
        if not current_user.is_authenticated:
            return {'_id': -1}  # Impossível
        
        if current_user.nivel_acesso == 'super_admin':
            return {}
        elif current_user.nivel_acesso == 'admin_central':
            # Tentar filtrar por _id (ObjectId) ou id (string/int)
            cid = current_user.central_id
            try:
                return {'_id': ObjectId(cid)}
            except:
                return {'id': cid}
        elif current_user.nivel_acesso == 'gerente_almox' and current_user.almoxarifado_id:
            # Precisa buscar o almoxarifado para saber a central
            try:
                a = extensions.mongo_db['almoxarifados'].find_one({'_id': ObjectId(current_user.almoxarifado_id)})
            except:
                a = extensions.mongo_db['almoxarifados'].find_one({'id': current_user.almoxarifado_id})
        ...
        # Outros níveis restritos à sua hierarquia superior
        # Para simplificar: se não é admin, geralmente vê apenas sua própria central contextual
        # Mas a lógica exata depende de como o frontend usa isso.
        # Por segurança, padrão restritivo se não cair nos casos acima:
        return {'_id': -1}

    @staticmethod
    def get_almoxarifados_filter():
        """Retorna filtro MongoDB para almoxarifados"""
        if not current_user.is_authenticated:
            return {'_id': -1}
        ...
        if current_user.nivel_acesso == 'super_admin':
            ...
            cid = current_user.central_id
            return {'central_id': cid} # Assume consistência de tipo (str/ObjectId)
        elif current_user.nivel_acesso == 'gerente_almox':
            aid = current_user.almoxarifado_id
            try:
                return {'_id': ObjectId(aid)}
            except:
                return {'id': aid}
    ...
    @staticmethod
    def get_sub_almoxarifados_filter():
        """Retorna filtro MongoDB para sub-almoxarifados"""
        if not current_user.is_authenticated:
            return {'_id': -1}
        ...
        if current_user.nivel_acesso == 'super_admin':
            ...
            # Todos os subs de almoxarifados da central
            cid = current_user.central_id
            # Busca IDs dos almoxarifados desta central
            a_ids = [doc['_id'] for doc in extensions.mongo_db['almoxarifados'].find({'central_id': cid}, {'_id': 1})]
            # Também buscar por 'id' string se houver mistura
            a_ids_str = [str(i) for i in a_ids]
            return {'almoxarifado_id': {'$in': a_ids + a_ids_str}}
        elif current_user.nivel_acesso == 'gerente_almox':
            return {'almoxarifado_id': current_user.almoxarifado_id}
        elif current_user.nivel_acesso == 'resp_sub_almox':
            sid = current_user.sub_almoxarifado_id
            try:
                return {'_id': ObjectId(sid)}
            except:
                return {'id': sid}
    ...
    @staticmethod
    def get_setores_filter():
        """Retorna filtro MongoDB para setores"""
        if not current_user.is_authenticated:
            return {'_id': -1}
        ...
        if current_user.nivel_acesso == 'super_admin':
            ...
            # Todos os setores de sub-almox de almox da central... ufa
            cid = current_user.central_id
            # 1. Buscar Almoxarifados da Central
            a_cursor = extensions.mongo_db['almoxarifados'].find({'central_id': cid}, {'_id': 1, 'id': 1})
            a_ids = []
            for d in a_cursor:
                a_ids.append(d.get('_id'))
                if d.get('id'): a_ids.append(d.get('id'))
            
            # 2. Buscar SubAlmoxarifados desses Almoxarifados
            s_cursor = extensions.mongo_db['sub_almoxarifados'].find({'almoxarifado_id': {'$in': a_ids}}, {'_id': 1, 'id': 1})
            s_ids = []
            for d in s_cursor:
                s_ids.append(d.get('_id'))
                if d.get('id'): s_ids.append(d.get('id'))
                
            return {'sub_almoxarifado_id': {'$in': s_ids}}
            
        elif current_user.nivel_acesso == 'gerente_almox':
            aid = current_user.almoxarifado_id
            # Buscar subs deste almox
            s_cursor = extensions.mongo_db['sub_almoxarifados'].find({'almoxarifado_id': aid}, {'_id': 1, 'id': 1})
            s_ids = []
            for d in s_cursor:
                s_ids.append(d.get('_id'))
                if d.get('id'): s_ids.append(d.get('id'))
            ...
            return {'sub_almoxarifado_id': current_user.sub_almoxarifado_id}
        elif current_user.nivel_acesso == 'operador_setor':
            sid = current_user.setor_id
            try:
                return {'_id': ObjectId(sid)}
            except:
                return {'id': sid}
    ...
    @staticmethod
    def get_estoque_filter():
        """Retorna filtro MongoDB para estoque"""
        # Estoque pode estar em 'estoques' ou ser uma query sobre produtos?
        # Assumindo coleção 'estoques' que tem campos local_tipo e local_id, ou almoxarifado_id etc.
        # O código original referenciava EstoqueProduto com campos almoxarifado_id, sub_almoxarifado_id, setor_id
        ...
        # Para simplificar a lógica complexa de hierarquia:
        # Se temos acesso a uma lista de locais, filtramos por eles.
        # Mas para estoque, muitas vezes o registro tem links diretos.
        
        if current_user.nivel_acesso == 'admin_central':
            # Estoque de qualquer local da central
            # Isso exigiria uma query complexa com $or ou pré-buscar todos os IDs de locais.
            # Simplificação: filtrar produtos/estoque que tenham central_id (se desnormalizado)
            # ou usar a lógica de filtros acima para compor.
            
            # Vamos usar a lógica de "onde está o estoque".
            # Se o estoque tem 'almoxarifado_id', 'sub_almoxarifado_id', 'setor_id'
            ...
            # Construir cláusula $or
            conditions = []
            if 'almoxarifado_id' in f_almox: conditions.append({'almoxarifado_id': f_almox['almoxarifado_id']})
            elif 'central_id' in f_almox: pass # Estoque geralmente não tem central_id direto, precisa resolver
            
            # Recalcular IDs para ser seguro
            cid = current_user.central_id
            # Almoxarifados
            a_ids = [d['_id'] for d in extensions.mongo_db['almoxarifados'].find({'central_id': cid}, {'_id': 1})]
            a_ids_str = [str(x) for x in a_ids]
            all_a = a_ids + a_ids_str
            
            # SubAlmoxarifados
            s_ids = [d['_id'] for d in extensions.mongo_db['sub_almoxarifados'].find({'almoxarifado_id': {'$in': all_a}}, {'_id': 1})]
            s_ids_str = [str(x) for x in s_ids]
            all_s = s_ids + s_ids_str
            
            # Setores
            se_ids = [d['_id'] for d in extensions.mongo_db['setores'].find({'sub_almoxarifado_id': {'$in': all_s}}, {'_id': 1})]
            se_ids_str = [str(x) for x in se_ids]
            all_se = se_ids + se_ids_str
            
            return {
                '$or': [
                    {'almoxarifado_id': {'$in': all_a}},
                    {'sub_almoxarifado_id': {'$in': all_s}},
                    {'setor_id': {'$in': all_se}}
                ]
            }
            ...
            aid = current_user.almoxarifado_id
            # Subs
            s_ids = [d['_id'] for d in extensions.mongo_db['sub_almoxarifados'].find({'almoxarifado_id': aid}, {'_id': 1})]
            # Setores
            se_ids = [d['_id'] for d in extensions.mongo_db['setores'].find({'sub_almoxarifado_id': {'$in': s_ids}}, {'_id': 1})]
            
            return {
                '$or': [
                    {'almoxarifado_id': aid},
                    {'sub_almoxarifado_id': {'$in': s_ids}},
                    {'setor_id': {'$in': se_ids}}
                ]
            }
            ...
            sid = current_user.sub_almoxarifado_id
            # Setores
            se_ids = [d['_id'] for d in extensions.mongo_db['setores'].find({'sub_almoxarifado_id': sid}, {'_id': 1})]
            return {
                '$or': [
                    {'sub_almoxarifado_id': sid},
                    {'setor_id': {'$in': se_ids}}
                ]
            }
            
        elif current_user.nivel_acesso == 'operador_setor':
            return {'setor_id': current_user.setor_id}
